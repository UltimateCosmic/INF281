...
void Lista::actualiza(ifstream &arch) {
    ...
    while (true) {
        arch>>dni;
        if (arch.eof()) break;
        arch>>c>>dd>>c>>mm>>c>>aaaa;
        actualizaCliente(dni, aaaa*10000+mm*100+dd);
    }
}
...
void Lista::actualizaCliente(int dni, int fecha) {
    class Nodo *p = lfin;
    while (p) {
        if (p->ped->GetDni()==dni && p->ped->GetFecha()<fecha) break;
        p = p->ant;
    } 
    while (p and p->ped->GetDni()==dni) {
        p->ped->descuentaFlete();
        p = p->ant;
    }
}
